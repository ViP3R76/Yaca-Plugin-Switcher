...
public partial class MainWindow
{
    ...
    private Grid RenderDashboard()
    {
        ...
        AddDashboardTile(actions, 0, DashboardIconRegistry.IconAssetSync, IsGerman ? "YACA WECHSELN" : "SWITCH YACA", IsGerman ? "Version auswählen\nund wechseln" : "Select a version\nand switch", (Brush)FindResource("AccentBrush"), () => ShowSwitchPage());
        AddDashboardTile(actions, 1, DashboardIconRegistry.IconAssetBackup, IsGerman ? "BACKUP ERSTELLEN" : "CREATE BACKUP", IsGerman ? "Aktuelle Version sichern" : "Save current version", (Brush)FindResource("GoldBrush"), CreateBackupFromDashboard);
        AddDashboardTile(actions, 2, DashboardIconRegistry.IconAssetUpdater, "YACA UPDATER", IsGerman ? "Neueste DLL prüfen\nund herunterladen" : "Check and download\nlatest DLL", (Brush)FindResource("AccentBrush"), () => ShowSwitchPage(IsGerman ? "YACA Updater: Download-Bereich geöffnet." : "YACA Updater: download area opened."));
    ...
    private void BuildCurrentInstalledPanel(Grid host, int column)
    {
        ...
        _currentDetails = new TextBlock { FontSize = 13, LineHeight = 20, Foreground = (Brush)FindResource("ForegroundBrush"), TextAlignment = TextAlignment.Left, HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Bottom, Margin = new Thickness(0, 8, 0, 0), TextWrapping = TextWrapping.NoWrap };
    ...
    private void BuildTeamSpeakPanel(Grid host, int column)
    {
        ...
        panel.RowDefinitions.Add(new RowDefinition { Height = new GridLength(1, GridUnitType.Star) });
        panel.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        var header = CreateDashboardHeader(DashboardIconRegistry.IconAssetTeamSpeak, "STATUS", gold);
        Grid.SetRow(header, 0);
        panel.Children.Add(header);
        ...
        _tsDescription = new TextBlock { FontSize = 14, Foreground = (Brush)FindResource("SecondaryBrush"), TextWrapping = TextWrapping.NoWrap, TextAlignment = TextAlignment.Center, Margin = new Thickness(0, 10, 0, 0), MaxWidth = 420 };
    ...
    {
        var button = new Button { Style = (Style)FindResource("TileButtonStyle"), BorderBrush = accent, Margin = new Thickness(6) };
        ...
        panel.Children.Add(new TextBlock { Text = title, FontSize = DashboardTileTitleFontSize, FontWeight = FontWeights.Bold, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 8, 0, 4) });
    ...
    private void BuildLatestBackupPanel(Grid host, int column)
    {
        ...
        panel.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        panel.RowDefinitions.Add(new RowDefinition { Height = new GridLength(1, GridUnitType.Star) });
        var header = CreateDashboardHeader(DashboardIconRegistry.IconAssetBackup, IsGerman ? "LETZTES BACKUP" : "LATEST BACKUP");
        Grid.SetRow(header, 0);
        panel.Children.Add(header);
        ...
        _backupSummary = new TextBlock { FontSize = 15, LineHeight = 22, Foreground = (Brush)FindResource("ForegroundBrush"), TextWrapping = TextWrapping.NoWrap, VerticalAlignment = VerticalAlignment.Top, HorizontalAlignment = HorizontalAlignment.Left, TextAlignment = TextAlignment.Left, Margin = new Thickness(0, 0, 10, 0) };
    ...
    private void UpdateBackupSummary(BackupInfo? backup)
    {
        ...
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run($"{versionText} • (Build: {buildText})") { FontSize = 20, FontWeight = FontWeights.SemiBold, Foreground = (Brush)FindResource("ForegroundBrush") });
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run(" ") { FontSize = 8 });
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run($"Status: {statusText}") { FontSize = 16, FontWeight = FontWeights.SemiBold, Foreground = (Brush)FindResource("ForegroundBrush") });
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run($"Größe: {sizeMb:0.00} MB ({sizeBytes.ToString("N0", CultureInfo.GetCultureInfo("de-DE"))} Bytes)") { FontSize = 15, Foreground = (Brush)FindResource("ForegroundBrush") });
    }
