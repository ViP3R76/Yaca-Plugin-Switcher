...
public partial class MainWindow : Window
{
    ...
    {
        _service = service ?? throw new ArgumentNullException(nameof(service)); InitializeComponent(); VersionText.Text = "v1.1.0"; BuildNavigation(); LoadLanguageSelector(); ShowHome();
    }
    ...
    private void AddNav(string key, string iconKey, string text, Action action)
    {
        ...
        if (DashboardIconData.TryGetValue(iconKey, out var data)) { icon = CreateIcon(data, (Brush)FindResource("ForegroundBrush"), 30, 30, iconKey.Equals("switch", StringComparison.OrdinalIgnoreCase) ? 4.0 : 2.35); content.Children.Add(icon); }
        var label = new TextBlock { Text = text, FontSize = 14, VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(12, 0, 0, 0) }; content.Children.Add(label);
        var button = new Button { Style = (Style)FindResource("NavButtonStyle"), Height = 46, Tag = key, Content = content };
        button.MouseEnter += (_, _) => { button.Background = (Brush)FindResource("NavSelectedBrush"); button.Foreground = (Brush)FindResource("GoldBrush"); button.BorderBrush = (Brush)FindResource("GoldBrush"); if (icon is not null) icon.Stroke = (Brush)FindResource("GoldBrush"); };
        button.MouseLeave += (_, _) => { var selected = string.Equals(_activePage, key, StringComparison.OrdinalIgnoreCase); button.Background = selected ? (Brush)FindResource("NavSelectedBrush") : Brushes.Transparent; button.Foreground = selected ? (Brush)FindResource("GoldBrush") : (Brush)FindResource("ForegroundBrush"); button.BorderBrush = selected ? (Brush)FindResource("GoldBrush") : Brushes.Transparent; if (icon is not null) icon.Stroke = selected ? (Brush)FindResource("GoldBrush") : (Brush)FindResource("ForegroundBrush"); };
    ...
    private void ShowHome() { _activePage = "home"; _pageStatus = null; _pageStatusBorder = null; PageHost.Content = RenderDashboard(); SetActiveNav("home"); RefreshHome(); }
