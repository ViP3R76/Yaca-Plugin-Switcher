...
public sealed class YacaUpdaterService
{
    ...
    public async Task<IReadOnlyList<(string Version, string FileName, long Size)>> GetAvailableDownloadsAsync(CancellationToken cancellationToken = default)
    {
        Directory.CreateDirectory(DownloadDirectory);
        var local = Directory.EnumerateFiles(DownloadDirectory, "*.ts3_plugin", SearchOption.TopDirectoryOnly)
    ...
    public async Task<IReadOnlyList<string>> GetMissingVersionsAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(_service.PluginsDirectory, "yaca_*_win64.dll"))
        {
    ...
    public async Task DownloadMissingAsync(IProgress<YacaUpdaterProgress>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var version in missing)
        {
            ...
            var url = string.Format(CdnBase, version);
            try
            {
    ...
    {
        progress?.Report(new(version, new FileInfo(archivePath).Length, new FileInfo(archivePath).Length, "Archiv wird geprüft", false, false, null));
        var tag = version.Replace(".", "");
        var target = Path.Combine(_service.PluginsDirectory, $"yaca_{tag}_win64.dll");
        ...
        try
        {
            ...
            progress?.Report(new(version, new FileInfo(archivePath).Length, new FileInfo(archivePath).Length, "DLL wird validiert", false, false, null));
            ...
            progress?.Report(new(version, new FileInfo(archivePath).Length, new FileInfo(archivePath).Length, "Erfolgreich hinzugefügt", true, true, null));
        }
        finally
        {
