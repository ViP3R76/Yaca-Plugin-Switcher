...
public partial class MainWindow
{
    ...
    private void BuildAvailableVersionsPanel(Grid host, int column)
    {
        ...
        var footerText = new TextBlock { Text = "0", FontSize = DashboardFooterFontSize, Foreground = (Brush)FindResource("AccentBrush"), Background = Brushes.Transparent, HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Center, Padding = new Thickness(8, 4, 8, 4), IsHitTestVisible = false }; Grid.SetColumn(footerText, 0); footer.Children.Add(footerText);
        ...
        footer.MouseLeftButtonUp += (_, _) => ShowSwitchPage(); Grid.SetRow(footer, 2); panel.Children.Add(footer); card.Child = panel; Grid.SetColumn(card, column); host.Children.Add(card); _versionsFooterText = footerText;
    }
