...
public partial class MainWindow
{
    ...
    /// <summary>
    /// Baut die Dashboard-Grundstruktur aus den drei vertikalen Bereichen auf.
    /// Die eigentlichen Panel-Inhalte werden in den dafür vorgesehenen Methoden erstellt.
    /// </summary>
    private Grid RenderDashboard()
    {
        ...
        {
            Height = new GridLength(DashboardPanelHeight)
        });
        root.RowDefinitions.Add(new RowDefinition
        {
            Height = new GridLength(DashboardPanelHeight)
        ...
        AddDashboardTile(
            ...
            CreateBackupFromDashboard);
        AddDashboardTile(
            actions,
            2,
            DashboardIconRegistry.IconAssetUpdater,
            "YACA UPDATER",
            IsGerman ? "Neueste DLL prüfen\nund herunterladen" : "Check and download\nlatest DLL",
            (Brush)FindResource("AccentBrush"),
            () => ShowSwitchPage(
                IsGerman
                    ? "YACA Updater: Download-Bereich geöffnet."
                    : "YACA Updater: download area opened."));
    ...
    /// <summary>
    /// Erstellt die einheitliche Kartenhülle für Dashboard-Panels.
    /// </summary>
    ...
    private void BuildCurrentInstalledPanel(Grid host, int column)
    {
        ...
        _currentDetails = new DashboardDetailsTextBlock(UpdateCurrentInstalledDetailsFromText)
        {
            Visibility = Visibility.Collapsed
        };

        _currentDetailsPanel = new Grid
        {
    ...
    {
        if (_currentDetailsPanel is null
            || _currentMetaText is null
            || _currentShaLabel is null
            || _currentShaValue is null)
        {
            return;
        }
    ...
    private void BuildTeamSpeakPanel(Grid host, int column)
    {
        ...
        var header = CreateDashboardHeader(
            DashboardIconRegistry.IconAssetTeamSpeakStatus,
            "TEAMSPEAK STATUS",
            gold);
        Grid.SetRow(header, 0);
        panel.Children.Add(header);
    ...
    private void BuildAvailableVersionsPanel(Grid host, int column)
    {
        ...
        var header = CreateDashboardHeader(
            DashboardIconRegistry.IconAssetBackups,
        ...
        {
            Data = Geometry.Parse("M 2 2 L 8 8 L 2 14"),
            Stroke = purple,
            StrokeThickness = 2.2,
    ...
    private Grid CreateDashboardHeader(
        string iconAssetKey,
        string text,
        Brush? headerBrush = null)
    {
        ...
        content.Children.Add(DashboardIconRegistry.CreateIcon(
            iconAssetKey,
            brush,
            DashboardHeaderIconSize,
            DashboardHeaderIconSize));
        content.Children.Add(new TextBlock
        {
            Text = text,
            FontSize = DashboardHeaderFontSize,
            FontWeight = FontWeights.SemiBold,
            Foreground = brush,
            VerticalAlignment = VerticalAlignment.Center,
        ...
        Action action)
    {
        ...
        panel.Children.Add(DashboardIconRegistry.CreateIcon(
            iconAssetKey,
            accent,
            DashboardTileIconSize,
            DashboardTileIconSize));
        panel.Children.Add(new TextBlock
        {
            Text = title,
            FontSize = DashboardTileTitleFontSize,
            FontWeight = FontWeights.Bold,
            Foreground = accent,
            HorizontalAlignment = HorizontalAlignment.Center,
        ...
        {
            Text = subtitle,
            FontSize = DashboardTileSubtitleFontSize,
            Foreground = (Brush)FindResource("SecondaryBrush"),
            TextAlignment = TextAlignment.Center,
            TextWrapping = TextWrapping.NoWrap
        });
    ...
    private void BuildLatestBackupPanel(Grid host, int column)
    {
        ...
        var backupIcon = DashboardIconRegistry.CreateIcon(
            DashboardIconRegistry.IconAssetBackup,
            purple,
            132,
            132);
    ...
    private void RenderVersionList(YacaPluginInfo? current)
    {
        ...
        for (var index = 0; index < ordered.Count; index++)
        {
            ...
            grid.Children.Add(new TextBlock
            {
                ...
                FontSize = DashboardVersionListFontSize,
                FontWeight = FontWeights.SemiBold,
                VerticalAlignment = VerticalAlignment.Center
            });
    ...
    private void UpdateBackupSummary(BackupInfo? backup)
    {
        ...
        var versionText = backup.DisplayName.Split(
            " - ",
            2,
        ...
        _backupSummary.Inlines.Add(new Run(" ")
        {
            FontSize = 8
        });
        _backupSummary.Inlines.Add(new LineBreak());
        ...
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run(
            $"Größe: {sizeMb:0.00} MB ({sizeBytes.ToString("N0", CultureInfo.GetCultureInfo("de-DE"))} Bytes)")
        {
            FontSize = 15,
            Foreground = foreground
        });
    ...
    /// <summary>
    /// Erstellt das einfache, rahmenlose Button-Template für den TeamSpeak-Schließen-Button.
    /// </summary>
    ...
    /// <summary>
    /// Fügt dem Grid die gewünschte Anzahl automatisch dimensionierter Zeilen hinzu.
    /// </summary>
...
/// <summary>
/// Erkennt Änderungen am Text des Dashboard-Detailsfeldes und leitet diese
/// an den zentralen Dashboard-Renderer weiter.
/// </summary>
