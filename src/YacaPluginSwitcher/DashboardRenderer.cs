...
public partial class MainWindow
{
    ...
    private void RenderVersionList(YacaPluginInfo? current)
    {
        ...
        for (var index = 0; index < ordered.Count; index++)
        {
            ...
            var row = new Border { Background = index % 2 == 0 ? Brushes.Transparent : (Brush)FindResource("SurfaceAltBrush"), CornerRadius = new CornerRadius(3), Padding = new Thickness(7, 2, 7, 2), Margin = new Thickness(0, 1, 0, 1) };
    ...
    private void UpdateBackupSummary(BackupInfo? backup)
    {
        ...
        _backupSummary.Inlines.Add(new Run($"{backup.Timestamp:dd.MM.yyyy HH:mm}") { FontSize = 31, FontWeight = FontWeights.SemiBold, Foreground = (Brush)FindResource("GoldBrush") }); _backupSummary.Inlines.Add(new LineBreak());
        ...
        _backupSummary.Inlines.Add(new Run($"{(IsGerman ? "Datei" : "File")} - {backup.FileName}") { FontSize = 14, Foreground = (Brush)FindResource("SecondaryBrush") }); _backupSummary.Inlines.Add(new LineBreak());
