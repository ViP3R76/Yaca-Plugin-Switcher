...
foreach($r in @('Assets\yaca_logo.png','Assets\discord_icon.png','Assets\github_icon.png','Assets\twitch_icon.svg')){if(-not(Test-Path (Join-Path $Root "src/YacaPluginSwitcher/$r") -PathType Leaf)){throw "Required asset missing: $r"}}
...
Write-Host 'C#, XAML, references, namespaces and analyzers are validated by dotnet build.' -ForegroundColor Cyan