...
public partial class MainWindow : Window
{
    ...
    {
        LanguageCombo.Items.Clear(); LanguageCombo.Items.Add(Texts.LanguageGerman); LanguageCombo.Items.Add(Texts.LanguageEnglish); LanguageCombo.SelectedIndex = IsGerman ? 0 : 1;
    }

    private void LanguageCombo_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (!IsInitialized || LanguageCombo.SelectedIndex < 0) return;
        var language = LanguageCombo.SelectedIndex == 0 ? Localization.German : Localization.English;
        if (string.Equals(Localization.Normalize(_service.Settings.Language), language, StringComparison.OrdinalIgnoreCase)) return;
        _service.Settings.Language = language; _service.Settings.Save();
        BuildNavigation(); LoadLanguageSelector(); ShowCurrentPageAfterLanguageChange();
    }

    private void ShowCurrentPageAfterLanguageChange()
    {
        switch (_activePage) { case "switch": ShowSwitchPage(); break; case "backups": ShowBackups(); break; case "config": ShowConfig(); break; case "info": ShowInfo(); break; default: ShowHome(); break; }
    }

    private void ShowHome()
    {
        _activePage = "home"; _pageStatus = null; _pageStatusBorder = null; PageHost.Content = RenderDashboard(); SetActiveNav("home"); RefreshHome();
    }
    ...
    private void RefreshHome(bool announce = false)
    {
        ...
        try
        {
            _plugins.Clear(); _plugins.AddRange(GetDistinctPlugins());
            var notice = announce ? GetNewPluginNotice(_plugins) : null;
            if (!_pluginBaselineInitialized) SetPluginBaseline(_plugins);
            var current = _service.DetectCurrent(); UpdateCurrentInstalled(current); if (announce) FlashElement(_currentCard);
    ...
    private void UpdateCurrentInstalled(YacaPluginInfo? current)
    {
        ...
        if (_currentDetails is not null) _currentDetails.Text = current is null ? string.Empty : $"Build: {current.Build?.ToString(CultureInfo.InvariantCulture) ?? "—"}\nGröße: {current.FileSize / 1024d / 1024d:0.00} MB\nSHA-256: {current.Sha256}\nDatei: {System.IO.Path.GetFileName(current.FilePath)}";
    }

    private List<YacaPluginInfo> GetDistinctPlugins()
    {
        var result = new List<YacaPluginInfo>(); var seen = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var p in _service.ScanPlugins()) if (seen.Add($"{p.FilePath}|{p.Sha256}")) result.Add(p); return result;
    }
    ...
    private string? GetNewPluginNotice(IReadOnlyList<YacaPluginInfo> plugins)
    {
        ...
        var added = plugins.Where(p => !_knownPlugins.Contains($"{p.FilePath}|{p.Sha256}")).ToList(); _knownPlugins = keys;
    ...
    {
        _activePage = "switch"; SetActiveNav("switch"); var root = new Grid(); root.RowDefinitions.Add(new RowDefinition { Height = new GridLength(54) }); root.RowDefinitions.Add(new RowDefinition { Height = new GridLength(1, GridUnitType.Star) }); root.RowDefinitions.Add(new RowDefinition { Height = new GridLength(62) }); root.Children.Add(new TextBlock { Text = IsGerman ? "YACA wechseln" : "Switch YACA", FontSize = 24, FontWeight = FontWeights.SemiBold, VerticalAlignment = VerticalAlignment.Center });
        var list = new StackPanel(); var scroll = new ScrollViewer { Content = list, VerticalScrollBarVisibility = ScrollBarVisibility.Auto }; Grid.SetRow(scroll, 1); root.Children.Add(scroll); var current = _service.DetectCurrent();
        ...
        _pageStatusBorder = new Border { Background = (Brush)FindResource("SurfaceBrush"), BorderBrush = (Brush)FindResource("BorderBrush"), BorderThickness = new Thickness(1), CornerRadius = new CornerRadius(8), Margin = new Thickness(6, 6, 6, 0), Padding = new Thickness(14, 0, 14, 0) }; _pageStatus = new TextBlock { Text = status ?? (IsGerman ? "Bereit." : "Ready."), FontSize = 14, Foreground = (Brush)FindResource("SecondaryBrush"), VerticalAlignment = VerticalAlignment.Center, TextAlignment = TextAlignment.Center }; _pageStatusBorder.Child = _pageStatus; Grid.SetRow(_pageStatusBorder, 2); root.Children.Add(_pageStatusBorder); PageHost.Content = root;
    }
