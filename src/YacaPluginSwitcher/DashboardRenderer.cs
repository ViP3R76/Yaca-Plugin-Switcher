...
public partial class MainWindow
{
    ...
    // Central icon definitions. The switch icon is the UXWing Sync icon geometry,
    // used by both navigation and the dashboard tile.
    ...
    // TeamSpeak Inline Light SVG supplied by the user. The complete mark, including
    // the TeamSpeak wordmark, is rendered as a filled vector in the panel header.
    ...
    private void BuildCurrentInstalledPanel(Grid host, int column)
    {
        ...
        var header = CreateDashboardHeader("shield", IsGerman ? "AKTUELL INSTALLIERT" : "CURRENTLY INSTALLED", gold);
        ...
        {
            Background = (Brush)FindResource("SuccessBrush"), CornerRadius = new CornerRadius(4),
            Padding = new Thickness(16, 5, 16, 5), Margin = new Thickness(0, 8, 0, 0),
        ...
        {
            FontSize = 13, LineHeight = 20, Foreground = gold, TextAlignment = TextAlignment.Left,
            HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Bottom,
            Margin = new Thickness(0, 8, 0, 0), TextWrapping = TextWrapping.NoWrap
        };
    ...
    private void BuildTeamSpeakPanel(Grid host, int column)
    {
        ...
        var header = CreateDashboardHeader("teamspeak", "STATUS", gold);
        Grid.SetRow(header, 0);
        panel.Children.Add(header);

        var center = new StackPanel { HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center };
        _tsStatus = new TextBlock { Text = "—", FontSize = 28, FontWeight = FontWeights.SemiBold, HorizontalAlignment = HorizontalAlignment.Center, Foreground = gold };
        ...
        _tsClose = new Button { Content = IsGerman ? "TeamSpeak 3 schließen" : "Close TeamSpeak 3", Visibility = Visibility.Collapsed, Style = (Style)FindResource("TileButtonStyle"), Foreground = gold, BorderBrush = gold, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 4, 0, 0) };
    ...
    private void BuildLatestBackupPanel(Grid host, int column)
    {
        ...
        Grid.SetRow(header, 0);
        panel.Children.Add(header);
        _backupSummary = new TextBlock { FontSize = 15, Foreground = (Brush)FindResource("ForegroundBrush"), TextWrapping = TextWrapping.Wrap, VerticalAlignment = VerticalAlignment.Top, TextAlignment = TextAlignment.Left, Margin = new Thickness(6, 4, 6, 6) };
    ...
    private void BuildAvailableVersionsPanel(Grid host, int column)
    {
        ...
        {
            Content = "0", FontSize = DashboardFooterFontSize, Foreground = (Brush)FindResource("AccentBrush"),
    ...
    private StackPanel CreateDashboardHeader(string iconKey, string text, Brush? headerBrush = null)
    {
        ...
        if (iconKey.Equals("teamspeak", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Data = Geometry.Parse(TeamSpeakLogoData), Fill = brush, Stroke = null,
                Width = 104, Height = DashboardHeaderIconSize, Stretch = Stretch.Uniform,
                HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Center
            });
        }
        else if (DashboardIconData.TryGetValue(iconKey, out var data))
        {
            header.Children.Add(CreateIcon(data, brush, DashboardHeaderIconSize, DashboardHeaderIconSize, iconKey.Equals("switch", StringComparison.OrdinalIgnoreCase) ? 3.0 : 2.15));
        }
        header.Children.Add(new TextBlock { Text = text, FontSize = DashboardHeaderFontSize, FontWeight = FontWeights.SemiBold, Foreground = brush, VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(10, 0, 0, 0), TextWrapping = TextWrapping.NoWrap });
        return header;
    }
    ...
    private void AddDashboardTile(Grid host, int column, string iconKey, string title, string subtitle, Brush accent, Action action)
    {
        ...
        if (DashboardIconData.TryGetValue(iconKey, out var data)) panel.Children.Add(CreateIcon(data, accent, DashboardTileIconSize, DashboardTileIconSize, iconKey.Equals("switch", StringComparison.OrdinalIgnoreCase) ? 6.0 : 3.6));
        panel.Children.Add(new TextBlock { Text = title, FontSize = DashboardTileTitleFontSize, FontWeight = FontWeights.SemiBold, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 8, 0, 4), TextWrapping = TextWrapping.NoWrap });
    ...
    private void RenderVersionList(YacaPluginInfo? current)
    {
        ...
        for (var index = 0; index < ordered.Count; index++)
        {
            ...
            var row = new Border
            {
                Background = index % 2 == 0 ? Brushes.Transparent : (Brush)FindResource("SurfaceAltBrush"),
                CornerRadius = new CornerRadius(3), Padding = new Thickness(7, 2, 7, 2), Margin = new Thickness(0, 1, 0, 1)
            };
    ...
    private void UpdateBackupSummary(BackupInfo? backup)
    {
        ...
        _backupSummary.Inlines.Add(new Run($"{backup.Timestamp:dd.MM.yyyy HH:mm}") { FontSize = 31, FontWeight = FontWeights.SemiBold, Foreground = (Brush)FindResource("GoldBrush") });
        _backupSummary.Inlines.Add(new LineBreak());
        _backupSummary.Inlines.Add(new Run($"YACA {backup.Version} - Backup-Status: {(backup.IsAutomatic ? (IsGerman ? "Automatisch" : "Automatic") : (IsGerman ? "Manuell" : "Manual"))}") { FontSize = 16, FontWeight = FontWeights.SemiBold, Foreground = (Brush)FindResource("ForegroundBrush") });
        _backupSummary.Inlines.Add(new LineBreak());
    ...
    private static System.Windows.Shapes.Path CreateIcon(string data, Brush stroke, double width, double height, double thickness) => new()
    {
        ...
        Width = width, Height = height, Stretch = Stretch.Uniform,
