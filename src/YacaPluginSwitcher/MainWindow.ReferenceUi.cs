...
public partial class MainWindow
{
    ...
    private void NavButton_MouseEnter(object sender, System.Windows.Input.MouseEventArgs e)
    {
        ...
        button.Background = (Brush)FindResource("NavSelectedBrush"); button.Foreground = (Brush)FindResource("GoldBrush"); button.BorderBrush = (Brush)FindResource("GoldBrush"); button.BorderThickness = new Thickness(1);
    }
    ...
    private void NavButton_MouseLeave(object sender, System.Windows.Input.MouseEventArgs e)
    {
        ...
        button.Background = selected ? (Brush)FindResource("NavSelectedBrush") : Brushes.Transparent; button.Foreground = selected ? (Brush)FindResource("GoldBrush") : (Brush)FindResource("ForegroundBrush"); button.BorderBrush = selected ? (Brush)FindResource("GoldBrush") : Brushes.Transparent; button.BorderThickness = selected ? new Thickness(1) : new Thickness(0);
    }
    ...
    private void ApplyReferencePanelIcons()
    {
        ...
        foreach (var text in FindVisualTextBlocks(PageHost).ToList())
        {
            ...
            if (!iconMap.TryGetValue(normalized, out var data) || text.Parent is not Panel parent || parent.Children.Count == 0) continue;
    ...
    private void RemoveUpdaterBadge()
    {
        ...
        {
            if (text.Parent is Border badge && badge.Parent is Panel parent) parent.Children.Remove(badge);
        }
    }
    ...
    private void ApplyReferenceDashboardGeometry()
    {
        ...
        var column = Grid.GetColumn(logo.Border); top.Children.Remove(logo.Border);
        ...
        Grid.SetColumn(freeLogo, column); top.Children.Add(freeLogo);
    }

    private void ApplyCurrentInstalledPanel()
    {
        if (_currentCard is null) return;
        if (_currentCard.Child is Grid existing && existing.Tag is string tag && tag.Equals("reference-current-panel", StringComparison.OrdinalIgnoreCase)) return;
        var current = _service.DetectCurrent();
        var oldValue = _currentValue?.Text ?? "—";
        var grid = new Grid { Tag = "reference-current-panel" };
        ...
        header.Children.Add(new TextBlock { Text = IsGerman ? "AKTUELL INSTALLIERT" : "CURRENTLY INSTALLED", Foreground = (Brush)FindResource("AccentBrush"), FontSize = 20, FontWeight = FontWeights.SemiBold, VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(10, 0, 0, 0) }); Grid.SetRow(header, 0); grid.Children.Add(header);
        var center = new StackPanel { HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center };
        _currentValue = new TextBlock { Text = current?.Version?.ToString() ?? oldValue, FontSize = 34, FontWeight = FontWeights.SemiBold, HorizontalAlignment = HorizontalAlignment.Center, TextAlignment = TextAlignment.Center }; center.Children.Add(_currentValue);
        ...
        _currentDetails = new TextBlock { Text = FormatCurrentInstalledDetails(current), FontSize = 13, LineHeight = 20, Foreground = (Brush)FindResource("SecondaryBrush"), HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center, TextAlignment = TextAlignment.Left, TextWrapping = TextWrapping.Wrap }; Grid.SetRow(_currentDetails, 2); grid.Children.Add(_currentDetails); _currentCard.Child = grid;
    }
    ...
    private string FormatCurrentInstalledDetails(YacaPluginSwitcher.Models.YacaPluginInfo? current)
    {
        ...
        var build = current.Build?.ToString(System.Globalization.CultureInfo.InvariantCulture) ?? "—"; var size = $"{current.FileSize / 1024d / 1024d:0.00} MB"; var sha = current.Sha256.Length > 12 ? current.Sha256[..12] + "…" : current.Sha256;
