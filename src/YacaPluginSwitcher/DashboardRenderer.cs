...
public partial class MainWindow
{
    ...
    {
        ["switch"] = "M 7,9 C 9,5 13,3 17,4 C 20,5 22,7 23,10 M 17,2 L 23,4 L 20,10 M 17,15 C 15,19 11,21 7,20 C 4,19 2,17 1,14 M 7,22 L 1,20 L 4,14",
    ...
    private void BuildCurrentInstalledPanel(Grid host, int column)
    {
        ...
        center.Children.Add(new Border { Background = (Brush)FindResource("SuccessBrush"), CornerRadius = new CornerRadius(4), Padding = new Thickness(14, 4, 14, 4), Margin = new Thickness(0, 10, 0, 0), HorizontalAlignment = HorizontalAlignment.Center, Child = new TextBlock { Text = IsGerman ? "AKTIV" : "ACTIVE", Foreground = Brushes.Black, FontSize = DashboardBadgeFontSize, FontWeight = FontWeights.Bold, TextAlignment = TextAlignment.Center } });
        Grid.SetRow(center, 1); panel.Children.Add(center);
        _currentDetails = new TextBlock { FontSize = 13, LineHeight = 20, Foreground = (Brush)FindResource("SecondaryBrush"), TextAlignment = TextAlignment.Left, HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Bottom, Margin = new Thickness(0, 0, 0, 2), TextWrapping = TextWrapping.Wrap };
        Grid.SetRow(_currentDetails, 1); panel.Children.Add(_currentDetails);
    ...
    private static void BuildFreeBrandLogo(Grid host, int column)
    {
        ...
        Grid.SetColumn(logo, column); host.Children.Add(logo);
    }
    ...
    private void BuildTeamSpeakPanel(Grid host, int column)
    {
        ...
        panel.RowDefinitions.Add(new RowDefinition { Height = new GridLength(1, GridUnitType.Star) });
        panel.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        var header = CreateDashboardHeader("teamspeak", "TEAMSpeak 3 STATUS"); Grid.SetRow(header, 0); panel.Children.Add(header);
        var center = new StackPanel { HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center };
        ...
        center.Children.Add(_tsStatus); center.Children.Add(_tsDescription); Grid.SetRow(center, 1); panel.Children.Add(center);
