...
public partial class MainWindow : Window
{
    ...
    private void SetGlobalStatus(string message, bool success = false) { GlobalFooterStatusText.Text = message; GlobalFooterStatusText.Foreground = (Brush)FindResource(success ? "SuccessBrush" : "ForegroundBrush"); GlobalFooterStatusText.FontWeight = success ? FontWeights.Bold : FontWeights.Normal; GlobalFooterStatusText.TextWrapping = TextWrapping.NoWrap; GlobalFooterStatusText.TextAlignment = TextAlignment.Center; }
    ...
    private void ShowSwitchPage(string? status = null)
    {
        ...
        var updateButton = new Button { Content = IsGerman ? "YACA UPDATES PRÜFEN UND HERUNTERLADEN" : "CHECK FOR YACA UPDATES AND DOWNLOAD", Height = 42, FontWeight = FontWeights.Bold, Background = (Brush)FindResource("GoldBrush"), Foreground = Brushes.Black, BorderThickness = new Thickness(0), Margin = new Thickness(6, 4, 6, 0), Cursor = Cursors.Hand }; updateButton.Click += async (_, _) => await RunUpdaterAsync(); Grid.SetRow(updateButton, 2); updaterPanel.Children.Add(updateButton);
        updaterCard.Child = updaterPanel; Grid.SetRow(updaterCard, 0); right.Children.Add(updaterCard);
